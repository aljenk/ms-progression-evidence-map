...
De eerste Pages-versie staat in `index.html` en `site/`. Het dashboard bevat een korte intro met statusuitleg en uitleg van bewijsniveau A tot en met E, filters voor status, categorie, bewijsniveau, MS-context en uitkomstmaat, en laadt `data/index.csv`, `data/expert-review-queue.csv` en de JSON-records direct vanuit de repository. De recorddetailweergave toont daarnaast een publicatiegate met disclaimer, claimgrens, volgende actie en bij `expert_review_requested` de open expertvraag. De workflow `.github/workflows/pages.yml` valideert de evidence map en publiceert daarna automatisch de `gh-pages` branch.
